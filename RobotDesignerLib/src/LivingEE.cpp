...
WheelEE_RMC::WheelEE_RMC(const char* LMType){
	...
	if (strcmp(trim((char*)LMType), "ActiveWheel") == 0) {
		isActive = true;
...
WheelEE_RMC* WheelEE_RMC::clone(){
	...
	if (originalWheelMesh)
		new_rmc->originalWheelMesh = originalWheelMesh->clone();
	if (motorMesh)
		new_rmc->motorMesh = motorMesh->clone();
	if (motorBracketMesh)
		new_rmc->motorBracketMesh = motorBracketMesh->clone();
...
bool WheelEE_RMC::pickMesh(Ray& ray, double* closestDist /*= NULL*/){
	...
	return (wheelMesh && wheelMesh->getDistanceToRayOriginIfHit(newRay, closestDist)
