...
private:
	...
	static T computeEnergyWheel(const Vector3T<T> &eePosjp1, const Vector3T<T> &eePosj, double dt,
		...
		Vector3T<T> wheelAxisWorld = LocomotionEngine_EndEffectorTrajectory::rotateWheelAxisWith(wheelAxisLocal, axisYaw, (T)0.5*(alphaj+alphajp1), axisTilt, (T)0.5*(betaj+betajp1));

		Vector3T<T> rhoWorld = LocomotionEngine_EndEffectorTrajectory::rotateWheelAxisWith(rhoLocal, axisYaw, (T)0.5*(alphaj+alphajp1), axisTilt, (T)0.5*(betaj+betajp1));
