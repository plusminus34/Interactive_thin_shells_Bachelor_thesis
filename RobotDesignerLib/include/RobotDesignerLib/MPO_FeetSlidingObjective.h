...
private:
	...
	template<class T>
	static T computeEnergy(const Vector3T<T> &eePosjp1, const Vector3T<T> &eePosj, double dt, const Vector3T<T> &wheelRadiusV, const T &wheelRadius, const Vector3T<T> &wheelAxis, const T &wheelAxisAlpha, const T &speed, double c, double weight) {
		Vector3T<T> axisRot = rotateVec(wheelAxis, wheelAxisAlpha, Vector3T<T>(0, 1, 0));
		Vector3T<T> rr = wheelRadiusV*wheelRadius;
		Vector3T<T> omega = axisRot*speed;
