...
void KS_AssemblyConstraintEnergy::initialize(KS_MechanicalAssembly* a){
	...
	C.resize(scalarConstraintCount, 0);
...
SparseMatrix* KS_AssemblyConstraintEnergy::getHessianAt(const dVector& s){
	...
	for (int i=0; i<ddE_dsds.getNumRows();i++)
...
SparseMatrix* KS_AssemblyConstraintEnergy::getConstraintJacobianAt(const dVector& s){
	assembly->setAssemblyState(s);
	dCds.setZero();
	for (uint i=0;i<constraints.size();i++)
		constraints[i]->writeConstraintJacobianValuesTo(dCds);
	return &dCds;
}
