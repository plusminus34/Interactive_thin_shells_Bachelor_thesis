...
void SQPFunctionMinimizer::computeGradient(ConstrainedObjectiveFunction *function, const dVector& pi) {
...
void SQPFunctionMinimizer::computeHessian(ConstrainedObjectiveFunction *function, const dVector& pi) {
	...
	smEntries.clear();
	function->getObjectiveFunction()->addHessianEntriesTo(smEntries, pi);
	H.setFromTriplets(smEntries.begin(), smEntries.end());
}

void SQPFunctionMinimizer::computeConstraintsAndJacobians(ConstrainedObjectiveFunction *function, const dVector& pi) {
	int nParameters = (int)pi.size();

	smEntries.clear();
	resize(A, function->getFunctionConstraints()->getEqualityConstraintCount(), nParameters);
	function->getFunctionConstraints()->addEqualityConstraintsJacobianEntriesTo(smEntries, pi);
...
bool SQPFunctionMinimizer::minimize(ConstrainedObjectiveFunction *function, dVector &p, double &functionValue){
	...
	for (i=0; i<maxIterations_; i++) {
		...
		if (printOutput_) Logger::logPrint("\t\t>>>>>>>> TIMERINFO: Time to compute gradient: %lf\n", sqpTimer.timeEllapsed());

		sqpTimer.restart();
		computeHessian(function, pi);
		if (printOutput_) Logger::logPrint("\t\t>>>>>>>> TIMERINFO: Time to compute hessian: %lf\n", sqpTimer.timeEllapsed());

		sqpTimer.restart();
		computeConstraintsAndJacobians(function, pi);
		if (printOutput_) Logger::logPrint("\t\t>>>>>>>> TIMERINFO: Time to compute constraints and their jacobians: %lf\n", sqpTimer.timeEllapsed());
		...
		computeSearchDirection(H,
		                       ...
		                       dp,
		                       A,
		                       function->getFunctionConstraints()->getEqualityConstraintsTargetValues(),
		                       function->getFunctionConstraints()->getInequalityConstraintsMinValues(),
		                       C,
		                       function->getFunctionConstraints()->getInequalityConstraintsMaxValues(),
		                       function->getFunctionConstraints()->getBoundConstraintsMinValues(),
		                       function->getFunctionConstraints()->getBoundConstraintsMaxValues());

		if (printOutput_) Logger::logPrint("\t\t>>>>>>>> TIMERINFO: Time to compute search direction: %lf\n", sqpTimer.timeEllapsed());
...
*/
		...
		sqpTimer.restart();
		// Do a line search
		double alpha = doLineSearch(function->getObjectiveFunction(), pi, dp, maxLineSearchIterations_);
		if (printOutput_) 
	...
	if (printOutput_) {
		...
		function->getFunctionConstraints()->printConstraintErrors(pi);
	}
...
void SQPFunctionMinimizer::checkConstraintValues(ConstrainedObjectiveFunction *function, const dVector &p, const dVector&dp) {
	// check to see if the solution is within the constraints.
	dVector pnext = p + dp;

	// p must satisfy A*p = b after the solve...
	if (A.cols() == p.size()) {
		dVector b = function->getFunctionConstraints()->getEqualityConstraintsTargetValues();
		dVector ap = A*p;
		dVector apComp = function->getFunctionConstraints()->getEqualityConstraintValues(p);
		dVector apNext = A*pnext;
		dVector apNextComp = function->getFunctionConstraints()->getEqualityConstraintValues(pnext);
...
//	and the 10000 vs inf issue...
	...
	if (C.cols() == p.size()) {
		dVector d = function->getFunctionConstraints()->getInequalityConstraintsMinValues();
		dVector f = function->getFunctionConstraints()->getInequalityConstraintsMaxValues();
		dVector cpComp = function->getFunctionConstraints()->getInequalityConstraintValues(p);
		dVector cp = C*p;
		dVector cpNext = C*pnext;
		dVector cpNextComp = function->getFunctionConstraints()->getInequalityConstraintValues(pnext);
	...
	dVector l = function->getFunctionConstraints()->getBoundConstraintsMinValues();
	dVector u = function->getFunctionConstraints()->getBoundConstraintsMaxValues();
...
void SQPFunctionMinimizer::computeSearchDirection(const SparseMatrix& hessian,
  ...
  double gradSearchDirDotProduct = -(gradient.dot(dp));
	...
	if (!success && gradSearchDirDotProduct <= 0){
		...
		tmpHes.setIdentity(); tmpHes *= 10000;
...
double SQPFunctionMinimizer::doLineSearch(ObjectiveFunction *function, dVector &p, const dVector &dp, int maxSteps){
	double alpha = 1.0;
	...
	const double initialValue = function->computeValue(p);
  dVector pc;
	...
	for (int j=0; j<maxSteps; j++)
	{
	  pc = p + alpha*dp;
		...
		const double newLineSearchValue = function->computeValue(pc);
