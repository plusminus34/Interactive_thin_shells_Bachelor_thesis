...
CMAFunctionMinimizer::CMAFunctionMinimizer(int maxIterations, int populationSize, double initialStdDev, double solveFunctionValue, double solveHistToleranceValue)
...
CMAFunctionMinimizer::~CMAFunctionMinimizer(){
}

void CMAFunctionMinimizer::setBounds(const dVector &pMin, const dVector &pMax){
	this->pMin = pMin;
	this->pMax = pMax;
}

bool CMAFunctionMinimizer::minimize(ObjectiveFunction *function, dVector &p, const dVector& startStdDev, double stdDevModifier, double &functionValue){
	...
	for (; !cmaes_TestForTermination(&evo); iter++)
	{
		...
		for (int popIdx=0; popIdx<popSize; popIdx++){
			...
			arFunVals[popIdx] = function->computeValue(pi);
		}
...
bool CMAFunctionMinimizer::minimize(ObjectiveFunction *function, dVector &p, double &functionValue){
	if (printLevel >= 1) {
