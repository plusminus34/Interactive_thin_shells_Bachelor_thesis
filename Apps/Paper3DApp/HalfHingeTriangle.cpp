#include "HalfHingeTriangle.h"
#include <GUILib/GLUtils.h>
#include <FEMSimLib/SimulationMesh.h>

HalfHingeTriangle::HalfHingeTriangle(SimulationMesh* simMesh, Node* n1, Node* n2, Node* n3, Node* n4) : SimMeshElement(simMesh) {
	this->n[0] = n1;
	this->n[1] = n2;
	this->n[2] = n3;
	this->n[3] = n4;
...
HalfHingeTriangle::~HalfHingeTriangle(){}

void HalfHingeTriangle::setRestShapeFromCurrentConfiguration() {
...
double HalfHingeTriangle::getMass() {
	return 0;
}

double HalfHingeTriangle::getAngle(const dVector& x) {
...
double HalfHingeTriangle::getEnergy(const dVector& x, const dVector& X) {
	double d_angle = getAngle(x) - restAngle;
	return k * 3 * restEdgeLength*restEdgeLength * d_angle*d_angle / restArea;
}

void HalfHingeTriangle::addEnergyGradientTo(const dVector& x, const dVector& X, dVector& grad) {
...
void HalfHingeTriangle::addEnergyHessianTo(const dVector& x, const dVector& X, std::vector<MTriplet>& hesEntries) {
	// constant factors
	double d_angle = getAngle(x) - restAngle;
	double K = k * 6 * restEdgeLength*restEdgeLength / restArea;
...
void HalfHingeTriangle::draw(const dVector& x) {
	/*
	P3D x0 = n[0]->getCoordinates(x);
	P3D x1 = n[1]->getCoordinates(x);
	P3D x2 = n[2]->getCoordinates(x);
...
void HalfHingeTriangle::drawRestConfiguration(const dVector& X) {
}
