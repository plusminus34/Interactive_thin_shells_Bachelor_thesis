...
PDEsDemoApp::~PDEsDemoApp(void){
}

//triggered when mouse moves
bool PDEsDemoApp::onMouseMoveEvent(double xPos, double yPos) {
...
bool PDEsDemoApp::onMouseButtonEvent(int button, int action, int mods, double xPos, double yPos) {
	isDrawing = false;
...
bool PDEsDemoApp::onMouseWheelScrollEvent(double xOffset, double yOffset) {
	if (GLApplication::onMouseWheelScrollEvent(xOffset, yOffset)) return true;

	return false;
}

bool PDEsDemoApp::onKeyEvent(int key, int action, int mods) {
	if (GLApplication::onKeyEvent(key, action, mods)) return true;

	return false;
}

bool PDEsDemoApp::onCharacterPressedEvent(int key, int mods) {
...
void PDEsDemoApp::loadFile(const char* fName) {
...
void PDEsDemoApp::saveFile(const char* fName) {
	Logger::consolePrint("SAVE FILE: Do not know what to do with file \'%s\'\n", fName);
}


// Run the App tasks
void PDEsDemoApp::process() {
	//do the work here...
...
double PDEsDemoApp::getColorFor(double val) {
if (runOption == HEAT_DIFFUSION_EQUATION)
	return 1 - val;
else
...
void PDEsDemoApp::drawAuxiliarySceneInfo() {

}

// Restart the application.
void PDEsDemoApp::restart() {

}

bool PDEsDemoApp::processCommandLine(const std::string& cmdLine) {
