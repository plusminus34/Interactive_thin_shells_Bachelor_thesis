...
IGL_INLINE void igl::matlab::mlsetmatrix(Engine** mlengine, std::string name, const std::vector<int> & M)
{
	if (*mlengine == 0)
		mlinit(mlengine);

	mxArray *A = mxCreateDoubleMatrix(M.size(), 1, mxREAL);
	double *pM = mxGetPr(A);
...
IGL_INLINE void igl::matlab::mlsetcellarray(Engine** mlengine, std::string name, const std::vector<std::vector<int>> M)
{

	if (*mlengine == 0)
		mlinit(mlengine);
	mxArray *cell = mxCreateCellMatrix(1, M.size());
	for (int i = 0; i < M.size(); i++)
	{
		mxArray *A = mxCreateDoubleMatrix(M[i].size(), 1, mxREAL);
		double *pM = mxGetPr(A);
	...
	engPutVariable(*mlengine, name.c_str(), cell);
	mxDestroyArray(cell);
}